...
struct Model {
    ...
    pool: Option<Arc<RefCell<vk::FixedSizeDescriptorSetsPool<Arc<vk::GraphicsPipelineAbstract + Send + Sync>>>>>,
...
fn view(app: &App, model: &Model, frame: Frame) -> Frame {
    ...
    let sub_buffer = model.buffer_pool.next(uniform_buffer).expect("Failed to get next uniform buffer");


    let descriptor_set = model.pool.as_ref().unwrap().borrow_mut().next()
        .add_buffer(sub_buffer)
        .expect("Failed to add uniform buffer")
        .build().expect("Failed to get next descriptor set");
