...
fn model(app: &App) -> Model {
    ...
    //let hue_values = (0..tile_count_x).map(|_| random() * 360.0).collect();
    ...
    for i in 0..tile_count_x as i32 {
    ...
    Model { window, tile_count_x, tile_count_y, hue_values, saturation_values, brightness_values }
}
                        ...
                        Key::Key0 => {
                            for i in 0..model.tile_count_x as i32 {
                                if i % 2 == 0 {
                                    ...
                                    model.saturation_values[i as usize] = random::<f32>() * 0.7 +
                                        0.3;
                                    model.brightness_values[i as usize] = random::<f32>() * 0.6 +
                                        0.4;
                                } else {
                                    model.hue_values[i as usize] = 0.58;
                                    model.saturation_values[i as usize] = random::<f32>() * 0.6 +
                                        0.4;
                                    model.brightness_values[i as usize] = random::<f32>() * 0.5 +
                                        0.5;
                                }
                            }
                        }
...
fn view(app: &App, model: &Model, frame: Frame) -> Frame {
    app.main_window().set_title("P_1_2_3_01");
    ...
    let mx = (app.mouse.x - win_rect.left()).max(1.0).min(app.window.width);
    let my = (win_rect.top() - app.mouse.y).max(1.0).min(app.window.height);

    // tile counter
    let mut counter = 0;

    // map mouse to grid resolution
    let current_tile_count_x = map_range(mx, 0.0, app.window.width, 1.0, model.tile_count_x as f32) as i32;
    let current_tile_count_y = map_range(my, 0.0, app.window.height, 1.0, model.tile_count_y as f32) as i32;
    let tile_width = app.window.width as i32 / current_tile_count_x;
    let tile_height = app.window.height as i32 / current_tile_count_y;
    ...
    while grid_y < model.tile_count_y {
        ...
        while grid_x < model.tile_count_x {
            let r = r.shift_x((tile_width * grid_x) as f32).shift_y(-(tile_height * grid_y) as f32);
            let index = counter % current_tile_count_x as usize;
            draw.rect().xy(r.xy()).wh(r.wh()).hsv(model.hue_values[index], model.saturation_values[index], model.brightness_values[index]);
            counter += 1;
            grid_x += 1;
        }
        grid_y += 1;
