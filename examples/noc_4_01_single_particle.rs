...
fn model(app: &App) -> Model {
    ...
    let p = Particle::new(Vector2::new(0.0,app.window.rect().top() - 20.0));
    Model { window, p }
}
...
fn event(app: &App, mut m: Model, event: Event) -> Model {
    match event {
        ...
        Event::Update(_dt) => {
            m.p.update();
            if m.p.is_dead() {
                m.p = Particle::new(Vector2::new(0.0,app.window.rect().top() - 20.0));
            }
        }
        _ => (),
    }
...
fn view(app: &App, m: &Model, frame: Frame) -> Frame {
    app.main_window().set_title("Single Particle");
