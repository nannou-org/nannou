...
use crate::{
    dac_manager::{DetectDacs, DetectedDacError, Result},
    util::{clamp, map_range},
    RawPoint,
};
...
pub(crate) fn detect_dacs() -> Result<DetectDacs> {
    ...
    let devices = helios_controller
        .list_devices()
        .map_err(DetectedDacError::from)?;
    if !devices.is_empty() {
        Ok(DetectDacs::Helios {
            previous_dac: devices.get(0).unwrap().0.into(),
        })
    } else {
