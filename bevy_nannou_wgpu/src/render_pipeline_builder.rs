...
pub struct RenderPipelineBuilder<'a> {
    ...
    vs_mod: Handle<Shader>,
    fs_mod: Option<Handle<Shader>>,
...
impl<'a> RenderPipelineBuilder<'a> {
    ...
    pub fn from_layout(layout: &'a [wgpu::BindGroupLayout], vs_mod: Handle<Shader>) -> Self {
        Self::new_inner(layout, vs_mod)
    }

    // Shared between constructors.
    fn new_inner(layout: &'a [wgpu::BindGroupLayout], vs_mod: Handle<Shader>) -> Self {
    ...
    pub fn fragment_shader(mut self, fs_mod: Handle<Shader>) -> Self {
    ...
    pub fn add_vertex_buffer_layout(mut self, d: wgpu::VertexBufferLayout) -> Self {
    ...
    pub fn build(self) -> wgpu::RenderPipelineDescriptor {
        ...
        let vertex = wgpu::VertexState {
            shader: vs_mod,
            entry_point: vs_entry_point.to_string().into(),
            buffers: Vec::from(&vertex_buffers[..]),
            shader_defs: vec![],
        };
        ...
        let fragment = match (fs_mod, color_states.is_empty()) {
            (Some(fs_mod), false) => Some(wgpu::FragmentState {
                shader: fs_mod,
                entry_point: fs_entry_point.to_string().into(),
                targets: Vec::from(color_states),
                shader_defs: vec![],
            }),
            _ => None,
        };
        ...
        wgpu::RenderPipelineDescriptor {
            label: Some(Cow::from("nannou render pipeline")),
