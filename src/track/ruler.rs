    ...
    fn update(self, args: widget::UpdateArgs<Self>) -> Self::Event {
        ...
        if let Some(mouse) = ui.widget_input(id).mouse() {
            if mouse.buttons.left().is_down() {
                ...
                let ticks = time::Ticks(((x / w) * total_ticks.ticks() as conrod::Scalar) as time::calc::Ticks);
        ...
        let markers_in_width_steps = markers_in_ticks.map(|markers| {
            markers.map(|ticks| ticks.beats(core::PPQN) * ruler.width_per_beat)
        });
        ...
        let iter = width_steps_with_divisions.enumerate().zip(state.ids.texts.iter());
